...
Итак, у нас есть два класса с разным набором полей, представляющие пользователя. В некоторых язык никакой проблемы вообще не возникнет, например в TypeScript.
```typescript
type UserDb = {
    id: number,
    name: string,
