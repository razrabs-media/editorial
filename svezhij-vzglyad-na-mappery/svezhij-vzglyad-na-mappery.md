...
Если вас, как и меня, не устраивает AutoMapper и его аналоги, но маппить вручную не хочется, приглашаю обсудить идею нового маппера, которого мы заслуживаем.

## Что такое маппинг и зачем он нужен?
Проще всего объяснить на примере, представим что у нас есть база данных с табличкой `Users`, в коде её представляет класс `User` у которого есть свойства `Id`, `Name`, `Email`, `CreatedAt`, и `IsDeleted`. Мы хотим написать апи, который позволит по `Id` получить пользователя. Но нам не нужны все поля, хранящиеся в базе, нас интересуют только `Name` и `Email`. Остальные поля - это технические детали.
Итак, у нас есть два класса с разным набором полей, представляющих пользователя. В некоторых языках никакой проблемы вообще не возникнет, например в TypeScript.
...
printUser(userDb); 
```
Это валидный код, и он будет делать именно то, что мы от него ожидаем. Компилятор сам понимает, что у `UserDb` есть поля `name` и `email`, поэтому может принять объект этого типа в метод `printUser`. Если мы изменим имя поля `name` в `UserDb`, то компилятор сообщит нам об ошибке: "Argument of type 'UserDb' is not assignable to parameter of type 'UserApi'. Property 'name' is missing in type 'UserDb' but required in type 'UserApi'". В C# Подобное невозможно из-за более строгой системы типов. Если у нас есть тип `A` и тип `B`, одинаковые у них поля или нет - не важно, для компилятора это будут разные типы, несовместимые друг с другом.
...
Может показаться, что написать метод для маппинга не так уж и сложно, но проблема в том, что даже в небольшом проекте, требуется написать десятки, иногда сотни таких методов. В одном из проектов мне пришлось почти весь день писать маппинги, и это было ужасно. Это рутинная работа, от которой мало кто может получать удовольствие. Поэтому мы используем AutoMapper и его аналоги, он позваляет сильно сократить количество написанного кода, избавляя нас от головной боли, особенно в простых случаях, когда модели не сильно отличаются друг от друга.
...
Я же вижу это так. Никаких конфигураций, просто метод `Map` из которого видно, что я хочу смаппить объект `source` в объект типа `UserDestination`.
```C#
using NextGenMapper;
...
Но почему бы не попробовать смаппить `SourceProperty` на `DestinationProperty`? Я хочу смаппить `Source` на `Destination`, разумно предположить, что это какие-то схожие типы, что свойства с одинаковыми именами должны маппиться. Конфигурация в таком случае будет не нужна.
```C#
using NextGenMapper;
...
Всё было бы куда проще, если типы для маппинга всегда были бы идентичны, свойства с одинаковыми именами, не отличающиеся по типу. Но мы живем в мире, где нам необходима возможность указать мапперу, как именно он должен маппить определённые типы.
...
var destination = source.MapWith<UserDestination>(
    ...
    email: src.Login);
```
И запомнить нужно только метод MapWith. 
...
```
Теперь этот метод могу использовать я, а также может использовать маппер, при маппинге других типов, например массива `UserSource[]` в `UserDestination[]`. Таким образом я как бы расширяю маппер, вручную добавляя в него метод.
...
Альтернативный подход, в котором настройка выглядит как обычный код, не сильно отличающийся от того, что я мог бы написать для ручного маппинга. 
```C#
using System;
...
Если говорить проще, используя сурс генераторы, я могу посмотреть, какой код написал пользователь, и проанализировав его, сгенерировать новый код, который будет добавлен прямо в сборку к пользователю, как будто он сам его написал.
...
Hello world!
```
Если мы изменим константу `HelloText`, класс `HelloWorld` перегенерируется. Важно понимать, что сурс генератор работает не только когда мы жмем на кнопку билда, компиляция происходит постоянно, пока мы пишем код.
